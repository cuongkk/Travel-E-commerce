...
const makeSlug = (text) => (0, slugify_1.default)(text, { lower: true, strict: true, locale: "vi" }) + "-" + rand(1000, 9999);
...
async function main() {
    ...
    await Promise.all([
        ...
        tour_model_1.default.deleteMany({}),
        gear_model_1.default.deleteMany({}),
        journal_model_1.default.deleteMany({}),
        account_model_1.default.deleteMany({}),
        voucher_model_1.default.deleteMany({}),
        order_model_1.default.deleteMany({}),
        review_model_1.default.deleteMany({}),
    ]);
    ...
    for (const cat of CATEGORY_DATA) {
        ...
        const existing = await category_model_1.default.findOne({ name: cat.name });
        if (existing) {
            categoryIds.push(existing._id.toString());
            log(`   ↷  Skip: ${cat.name}`);
        }
        else {
            const created = await category_model_1.default.create({ ...cat, slug, status: "active" });
    ...
    for (const t of TOUR_TEMPLATES) {
        const existing = await tour_model_1.default.findOne({ name: t.name });
        if (existing) {
        ...
        const created = await tour_model_1.default.create({
            name: t.name,
    ...
    for (const g of GEAR_DATA) {
        const existing = await gear_model_1.default.findOne({ name: g.name });
        if (existing) {
            gearIds.push(existing._id.toString());
            log(`   ↷  Skip: ${g.name}`);
            continue;
        }
        const created = await gear_model_1.default.create({ ...g, status: "active", deleted: false, rating: 0, reviewCount: 0 });
    ...
    for (const j of JOURNAL_DATA) {
        const existing = await journal_model_1.default.findOne({ title: j.title });
        if (existing) {
            log(`   ↷  Skip: ${j.title.substring(0, 40)}...`);
            continue;
        }
        await journal_model_1.default.create({ ...j, status: "active", deleted: false });
        log(`   ✅ Created: ${j.title.substring(0, 40)}...`);
    }
    // ────── ADMIN ACCOUNTS ──────
    section("5a/8  ADMIN ACCOUNTS");
    const hashedPass = await bcryptjs_1.default.hash("Ccuong123", 10);
    for (const adm of ADMIN_DATA) {
        const existing = await account_model_1.default.findOne({ email: adm.email });
        if (existing) {
    ...
    for (const acc of ACCOUNT_DATA) {
        const existing = await account_model_1.default.findOne({ email: acc.email });
        if (existing) {
        ...
        const created = await account_model_1.default.create({
            ...acc,
            password: hashedPass,
            role: "client",
            status: "active",
    ...
    for (const v of VOUCHER_DATA) {
        const existing = await voucher_model_1.default.findOne({ code: v.code });
        if (existing) {
            log(`   ↷  Skip: ${v.code}`);
            continue;
        }
        await voucher_model_1.default.create({ ...v, deleted: false, usedCount: 0 });
    ...
    for (let i = 0; i < 300; i++) {
        ...
        if (tourIds.length > 0 && Math.random() > 0.3) {
            ...
            const tourDoc = TOUR_TEMPLATES[tourIds.indexOf(id)] || TOUR_TEMPLATES[0];
            const qty = rand(1, 4);
        ...
        ordersToInsert.push({
            ...
            accountId: accId,
            fullName: (accData === null || accData === void 0 ? void 0 : accData.fullName) || "Khách vãng lai",
            phone: (accData === null || accData === void 0 ? void 0 : accData.phone) || `090${rand(1000000, 9999999)}`,
            items,
            subTotal,
            discount,
            total,
    ...
    const inserted = await order_model_1.default.insertMany(ordersToInsert, { timestamps: false });
    ...
    const doneOrders = await order_model_1.default.find({ status: "done", deleted: false }).limit(150).lean();
    const reviewsToInsert = [];
    for (let i = 0; i < Math.min(150, doneOrders.length + 20); i++) {
        const order = i < doneOrders.length ? doneOrders[i] : null;
        let itemId = null;
        let itemType = "tour";
        if (((_a = order === null || order === void 0 ? void 0 : order.items) === null || _a === void 0 ? void 0 : _a.length) > 0) {
            const it = pick(order.items);
            itemId = ((_b = it.id) === null || _b === void 0 ? void 0 : _b.toString()) || ((_d = (_c = it._id) === null || _c === void 0 ? void 0 : _c.toString()) !== null && _d !== void 0 ? _d : null);
            itemType = it.type || "tour";
        }
        ...
        const accountId = (order === null || order === void 0 ? void 0 : order.accountId) || (accountIds.length > 0 ? pick(accountIds) : null);
        if (!itemId || !accountId)
            continue;
    ...
    if (reviewsToInsert.length > 0) {
        ...
        const bulkReviews = reviewsToInsert.map((r, idx) => ({
            updateOne: {
                filter: { _id: createdReviews[idx]._id },
                update: { $set: { createdAt: r.createdAt } },
            },
        }));
        await review_model_1.default.bulkWrite(bulkReviews);
        ...
        for (const [key, ratings] of itemMap) {
            ...
            const Model = type === "tour" ? tour_model_1.default : gear_model_1.default;
    ...
    // ────── SUMMARY ──────
    const [totalCat, totalTours, totalGears, totalJournals, totalAccs, totalOrders, totalVouchers, totalReviews] = await Promise.all([
        category_model_1.default.countDocuments(),
        tour_model_1.default.countDocuments({ deleted: false }),
        gear_model_1.default.countDocuments({ deleted: false }),
        journal_model_1.default.countDocuments({ deleted: false }),
        account_model_1.default.countDocuments({ deleted: false }),
        order_model_1.default.countDocuments({ deleted: false }),
        voucher_model_1.default.countDocuments({ deleted: false }),
        review_model_1.default.countDocuments({ deleted: false }),
    ]);
    const revenueAgg = await order_model_1.default.aggregate([
    ...
    console.log(`║  💰 Doanh thu    : ${((((_e = revenueAgg[0]) === null || _e === void 0 ? void 0 : _e.total) || 0) / 1e6).toFixed(1)}M đ${" ".repeat(22)}║`);
    console.log("╠═══════════════════════════════════════════════╣");
