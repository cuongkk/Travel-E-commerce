...
export const generateTourSchedule = async (name: string, time: string): Promise<any[]> => {
...
export const generateJournalContent = async (title: string, category: string): Promise<string> => {
  if (!genAI) throw new Error("Hệ thống chưa được cấu hình GEMINI_API_KEY.");
